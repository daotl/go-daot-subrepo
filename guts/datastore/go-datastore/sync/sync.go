...
func (d *MutexDatastore) Put(ctx context.Context, key key.Key, value []byte) (err error) {
	d.Lock()
	defer d.Unlock()
...
func (d *MutexDatastore) Sync(ctx context.Context, prefix key.Key) error {
	d.Lock()
	defer d.Unlock()
...
func (d *MutexDatastore) Get(ctx context.Context, key key.Key) (value []byte, err error) {
	d.RLock()
	defer d.RUnlock()
...
func (d *MutexDatastore) Has(ctx context.Context, key key.Key) (exists bool, err error) {
	d.RLock()
	defer d.RUnlock()
...
func (d *MutexDatastore) GetSize(ctx context.Context, key key.Key) (size int, err error) {
	d.RLock()
	defer d.RUnlock()
...
func (d *MutexDatastore) Delete(ctx context.Context, key key.Key) (err error) {
	d.Lock()
	defer d.Unlock()
...
func (b *syncBatch) Put(ctx context.Context, key key.Key, val []byte) error {
	b.mds.Lock()
	defer b.mds.Unlock()
	return b.batch.Put(ctx, key, val)
}

func (b *syncBatch) Delete(ctx context.Context, key key.Key) error {
	b.mds.Lock()
	defer b.mds.Unlock()
