...
func (dds *Delayed) Put(ctx context.Context, key key.Key, value []byte) (err error) {
	dds.delay.Wait()
	return dds.ds.Put(ctx, key, value)
}

// Sync implements Datastore.Sync
func (dds *Delayed) Sync(ctx context.Context, prefix key.Key) error {
	dds.delay.Wait()
	return dds.ds.Sync(ctx, prefix)
}

// Get implements the ds.Datastore interface.
func (dds *Delayed) Get(ctx context.Context, key key.Key) (value []byte, err error) {
	dds.delay.Wait()
	return dds.ds.Get(ctx, key)
}

// Has implements the ds.Datastore interface.
func (dds *Delayed) Has(ctx context.Context, key key.Key) (exists bool, err error) {
	dds.delay.Wait()
	return dds.ds.Has(ctx, key)
}

// GetSize implements the ds.Datastore interface.
func (dds *Delayed) GetSize(ctx context.Context, key key.Key) (size int, err error) {
	dds.delay.Wait()
	return dds.ds.GetSize(ctx, key)
}

// Delete implements the ds.Datastore interface.
func (dds *Delayed) Delete(ctx context.Context, key key.Key) (err error) {
	dds.delay.Wait()
